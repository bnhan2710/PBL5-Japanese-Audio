...
const GenerateJapaneseVoicePage: React.FC = () => {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      toast({ title: 'Lỗi', description: 'Không thể tải file, vui lòng thử lại', variant: 'destructive' });
    }
  };
  ...
  const handleGenerate = async () => {
    if (dialogues.length === 0) {
      toast({ title: 'Cảnh báo', description: 'Vui lòng nhập kịch bản để tạo audio', variant: 'destructive' });
      return;
    }
    ...
    try {
      ...
      toast({ title: 'Lỗi', description: error.message || 'Lỗi khi tạo audio', variant: 'destructive' });
      setPipelineStep(0);
    } finally {
        ...
        <div className="lg:col-span-7 flex flex-col gap-6">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-blue-600 dark:text-blue-400 mt-1 leading-relaxed">
                  Nhập kịch bản theo cú pháp <span className="font-semibold bg-blue-100 dark:bg-blue-800/50 px-1.5 py-0.5 rounded text-blue-800 dark:text-blue-200">Nhãn: Lời thoại</span>.
                  Ví dụ: <code className="font-mono bg-blue-100 dark:bg-blue-800/50 px-1.5 py-0.5 rounded text-blue-800 dark:text-blue-200">男2: こんにちは</code>.
                  Các dòng không chứa dấu hai chấm (:) sẽ tự động được xếp vào nhãn <b>Người dẫn chuyện</b>.
                </p>
              </div>
            </div>
            ...
            <div className="border-t border-blue-200/50 dark:border-blue-800/50 pt-4 grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xs font-mono font-medium text-blue-600 bg-blue-100 dark:bg-blue-800 px-2 py-0.5 rounded">{narratorPause.toFixed(1)}s</span>
                </div>
                <Slider
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  <Label className="text-sm font-semibold text-blue-800 dark:text-blue-300">Nghỉ Hội thoại (Dialogue)</Label>
                  <span className="text-xs font-mono font-medium text-blue-600 bg-blue-100 dark:bg-blue-800 px-2 py-0.5 rounded">{dialoguePause.toFixed(1)}s</span>
                </div>
                <Slider
          ...
          <div className="pt-2">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-xl font-bold tracking-tight text-slate-800 dark:text-slate-100">Reference Audio & Config</h2>
            </div>
            ...
            <CardContent className="p-6 space-y-6">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-slate-500">
                  Phát hiện <b>{speakers.length}</b> nhân vật và <b>{dialogues.length}</b> câu thoại.
                </p>
              </div>
              ...
              {isGenerating && pipelineStep > 0 && (
                <div className="mt-4 p-5 rounded-xl border border-blue-100 bg-blue-50/50 dark:bg-blue-900/10 dark:border-blue-800/40 space-y-4 animate-in slide-in-from-top-4 duration-300">
                  <div className="flex justify-between text-xs font-semibold text-blue-700 dark:text-blue-400 mb-1 tracking-wider uppercase">
                    ...
                    <span>{pipelineStep === 1 ? '15%' : pipelineStep === 2 ? '65%' : pipelineStep === 3 ? '95%' : '100%'}</span>
                  </div>
                  <div className="h-2 w-full bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden shadow-inner">
                    <div
                      ...
                      style={{ width: pipelineStep === 1 ? '15%' : pipelineStep === 2 ? '65%' : pipelineStep === 3 ? '95%' : '100%' }}
                    >
                  ...
                  <div className="flex flex-col gap-3 pt-2">
                    <div className={`flex items-center gap-3 text-sm transition-colors duration-300 ${pipelineStep >= 1 ? 'text-blue-700 dark:text-blue-400 font-semibold' : 'text-slate-400 dark:text-slate-600'}`}>
                      {pipelineStep > 1 ? <CheckCircle2 className="w-5 h-5 text-emerald-500 drop-shadow-sm" /> : pipelineStep === 1 ? <Loader2 className="w-5 h-5 animate-spin text-blue-500" /> : <Circle className="w-5 h-5 opacity-50" />}
                      1. Chuẩn bị kịch bản & thiết lập model
                    </div>
                    <div className={`flex items-center gap-3 text-sm transition-colors duration-300 ${pipelineStep >= 2 ? 'text-blue-700 dark:text-blue-400 font-semibold' : 'text-slate-400 dark:text-slate-600'}`}>
                      {pipelineStep > 2 ? <CheckCircle2 className="w-5 h-5 text-emerald-500 drop-shadow-sm" /> : pipelineStep === 2 ? <Loader2 className="w-5 h-5 animate-spin text-blue-500" /> : <Circle className="w-5 h-5 opacity-50" />}
                      2. Tổng hợp giọng nói đa nhân vật
                    </div>
                    <div className={`flex items-center gap-3 text-sm transition-colors duration-300 ${pipelineStep >= 3 ? 'text-blue-700 dark:text-blue-400 font-semibold' : 'text-slate-400 dark:text-slate-600'}`}>
                      {pipelineStep > 3 ? <CheckCircle2 className="w-5 h-5 text-emerald-500 drop-shadow-sm" /> : pipelineStep === 3 ? <Loader2 className="w-5 h-5 animate-spin text-blue-500" /> : <Circle className="w-5 h-5 opacity-50" />}

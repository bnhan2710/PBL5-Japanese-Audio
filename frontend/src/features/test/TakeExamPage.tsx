...
export function TakeExamContent({
  ...
  const headerAccentClass = variant === 'arena' ? 'text-orange-950' : 'text-teal-950'
  const timerToneClass =
    variant === 'arena' ? 'rounded-full bg-orange-100 px-5 py-3 text-orange-800' : ''
  ...
  const activeQuestion =
    exam?.questions.find((question) => question.question_id === activeQuestionId) || null
  const activeIndex =
    exam?.questions.findIndex((question) => question.question_id === activeQuestionId) ?? -1

  const answeredCount =
    exam?.questions.filter((question) => answers[question.question_id]).length || 0
    ...
    return (
      <div className="mx-auto max-w-3xl rounded-3xl border border-red-200 bg-red-50 px-8 py-10 text-center">
        <p className="text-lg font-semibold text-red-700">
          {error || 'Không thể hiển thị bài thi'}
        </p>
        <Button
          className="mt-5"
          variant="outline"
  ...
  return (
    <div className={standalone ? 'mx-auto max-w-[1650px]' : 'mx-auto max-w-[1500px]'}>
          ...
          <div className="grid items-center gap-4 lg:grid-cols-[1fr_auto_1fr]">
            <div className="flex min-w-0 items-center gap-4">
              ...
              <div className="min-w-0">
                <p className={`truncate text-base font-black tracking-tight ${headerAccentClass}`}>
                  {exam.title}
                </p>
          ...
          <div className="grid items-start gap-6 lg:grid-cols-[230px_minmax(0,1fr)] xl:grid-cols-[245px_minmax(0,1fr)]">
            <aside className="space-y-6">
              <div className="inline-flex items-center gap-1 rounded-2xl bg-emerald-100 px-4 py-2 text-[11px] font-bold text-emerald-900 sm:text-xs">
                Đã làm <span className="text-emerald-500">{answeredCount}</span>/
      ...
      {selectedAudioMode === 'simulation' && simulationAudioSrc && (
        <audio
          ref={simulationAudioRef}
          src={simulationAudioSrc}
          preload="auto"
          className="hidden"
        />
      )}
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-slate-950/45 p-4 backdrop-blur-sm">
          <div className="w-full max-w-xl rounded-[32px] border border-border bg-card p-8 shadow-2xl">
            ...
            <p className="mt-3 text-base leading-7 text-muted-foreground">
              Kết quả đã được lưu vào hệ thống. Bạn có thể quay lại danh sách đề thi hoặc xem lại
              điểm số tổng hợp bên dưới.
            </p>
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-3">
              ...
              <div className="rounded-2xl border border-border bg-slate-50 px-4 py-4 text-foreground">
                ...
                <div className="flex items-baseline gap-2 mt-1">
                  <p className="text-3xl font-black">{result.correct_answers}</p>
          ...
          <div className="w-full max-w-md rounded-[32px] border border-border bg-card p-8 text-center shadow-2xl">
            {startPhase === 'ready' ? (
              ...
              <>
                ...
                <div className="mt-4 text-7xl font-black leading-none text-blue-600">
                  {countdownSeconds}
                </div>
                <p className="mt-4 text-base text-muted-foreground">
                  Audio sẽ bắt đầu ngay sau khi đếm ngược kết thúc.
                </p>
              </>
            )}
          </div>

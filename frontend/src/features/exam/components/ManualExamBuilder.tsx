...
export const ManualExamBuilder: React.FC = () => {
  ...
  // --- PHẦN SỬA: Tính toán số thứ tự chuẩn JLPT ---
  const displayNumbers: Record<number, number> = {};
  let currentNumber = 1;

  // 1. Sắp xếp các Key của nhóm Mondai theo số (Ví dụ: Mondai 1 -> Mondai 2 -> Mondai 10)
  ...
  // 2. Duyệt qua các nhóm đã sắp xếp và gán số thứ tự liên tục
  sortedGroupKeys.forEach((groupKey) => {
    const questionsInGroup = groupedQuestions[groupKey];

    // Sắp xếp các câu trong cùng một nhóm dựa trên index gốc để giữ thứ tự nhập liệu
    const sortedQs = [...questionsInGroup].sort((a, b) => a.idx - b.idx);

    sortedQs.forEach(({ idx }) => {
      displayNumbers[idx] = currentNumber++;
    });
  });
  // ----------------------------------------------
        ...
        <div className="flex flex-col md:flex-row gap-6 mt-4 md:h-[700px]">
          ...
          {activeQ ? (
            <QuestionEditor
              question={activeQ}
              index={activeQIdx}
              displayNumber={displayNumbers[activeQIdx]}
            ...
            {activeQ && (
              <div className="flex-1 flex flex-col bg-white dark:bg-slate-800 shadow-sm rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
                <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between bg-slate-50/30 dark:bg-slate-800/50">
                  <h2 className="text-lg font-bold text-slate-800 dark:text-slate-100">Câu {displayNumbers[activeQIdx]} Preview</h2>
                </div>

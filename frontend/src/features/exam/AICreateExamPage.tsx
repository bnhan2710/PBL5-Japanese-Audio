...
function Step1({ audioFile, setAudioFile, level, setLevel, title, setTitle, description, setDescription, onNext, loading }: Step1Props) {
 ...
 <span className="text-blue-600 dark:text-blue-400">Click để tải lên</span> hoặc kéo thả file
 </p>
 ...
 <div>
 <label className="block text-sm font-bold text-slate-700 dark:text-muted-foreground mb-3">Cấp độ JLPT</label>
 ...
 <div>
 <label className="block text-sm font-bold text-slate-700 dark:text-muted-foreground mb-3">Tiêu đề đề thi</label>
 <input
 value={title}
 ...
 <div>
 <label className="block text-sm font-bold text-slate-700 dark:text-muted-foreground mb-3">Mô tả đề thi</label>
 <textarea
 value={description}
 ...
 <div>
 <p className="text-sm font-bold text-blue-800 dark:text-blue-300">Pipeline Local Reazon</p>
...
function Step2Processing({ jobId, onDone, onFailed }: Step2Props) {
 ...
 <span className={`text-sm font-medium flex-1 ${isDoneStep ? 'text-emerald-700 dark:text-emerald-400' : isActive ? 'text-blue-700 dark:text-blue-300' : 'text-muted-foreground'}`}>
 {s.label}
 </span>
 {isActive && !isFailed && <Loader2 className="w-4 h-4 text-blue-500 animate-spin shrink-0" />}
 </div>
 )
 })}
 </div>
...
function Step3Review({ editableQuestions, setEditableQuestions, audioFile }: Step3Props) {
 ...
 <span className="text-[10px] font-semibold text-muted-foreground bg-muted px-2 py-0.5 rounded-md">{qs.length} câu</span>
 </div>
 ...
 <button onClick={() => updateQuestion(activeQIdx, { question_number: Math.max(1, activeQ.question_number - 1) })} className="w-5 h-5 flex items-center justify-center bg-muted rounded text-muted-foreground hover:bg-slate-300 font-bold select-none leading-none">-</button>
 <span className="w-6 text-center">{activeQ.question_number}</span>
 <button onClick={() => updateQuestion(activeQIdx, { question_number: activeQ.question_number + 1 })} className="w-5 h-5 flex items-center justify-center bg-muted rounded text-muted-foreground hover:bg-slate-300 font-bold select-none leading-none">+</button>
 </div>
 </span>
 </div>
 </div>
 ...
 <button onClick={() => setIsEditingAudio(true)} className="text-xs px-3 py-1.5 rounded-lg bg-slate-100 hover:bg-muted text-muted-foreground font-medium shrink-0 flex items-center gap-1.5 transition-colors">
 ...
 >
 <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${a.is_correct ? 'border-emerald-500 bg-emerald-500' : 'border-border'
 }`}>
 ...
 <span className="text-[10px] font-medium text-muted-foreground mt-1 opacity-0 group-hover/answer:opacity-100 transition-opacity">Chọn</span>
 )}
 </button>
 <div className={`flex-1 border rounded-xl px-4 py-3 transition-colors ${a.is_correct ? 'border-emerald-400 bg-emerald-50/50 dark:border-emerald-500/50 dark:bg-emerald-900/10' : 'border-border bg-card hover:border-border'
 }`}>
 <div className="flex items-center gap-3">
 <span className={`text-sm font-bold ${a.is_correct ? 'text-emerald-500' : 'text-muted-foreground'}`}>{ai + 1}.</span>
 <input
 value={a.content}
...
function Step4Save({ questions, level, title, description, draftId, onBack }: Step4Props) {
 ...
 <div>
 <h3 className="text-base font-bold text-card-foreground">[{level}] {title}</h3>
 ...
 <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${a.is_correct ? 'border-emerald-500 bg-emerald-500' : 'border-border'}`}>
 {a.is_correct && <span className="w-2.5 h-2.5 rounded-full bg-card" />}
 </div>
 {a.is_correct && (
 <span className="text-[10px] font-bold text-emerald-600 mt-1">Đúng</span>
 ...
 <div className={`flex-1 border rounded-xl px-4 py-3 ${a.is_correct ? 'border-emerald-400 bg-emerald-50/50 dark:border-emerald-500/50 dark:bg-emerald-900/10' : 'border-border bg-muted'}`}>
 <div className="flex items-center gap-3">
 <span className={`text-sm font-bold ${a.is_correct ? 'text-emerald-500' : 'text-muted-foreground'}`}>{ai + 1}.</span>
 ...
 <div className="flex-1 flex flex-col items-center justify-center bg-card rounded-2xl border border-border">
 <Brain className="w-12 h-12 text-muted-foreground dark:text-muted-foreground mb-4" />
 <p className="text-muted-foreground font-medium">Chọn một câu hỏi ở danh sách bên trái để xem</p>
 </div>
 )}
 </div>
 </div>
...
export default function AICreateExamPage() {
 ...
 <button onClick={handleSaveDraft} disabled={savingDraft} className="flex items-center gap-2 px-5 py-2.5 bg-card border border-border text-muted-foreground rounded-xl text-sm font-bold hover:bg-accent hover:text-accent-foreground transition-colors shadow-sm disabled:opacity-50">

...
function AudioTrimmer({ audioFile, initialStart, initialEnd, onSave, onCancel }: AudioTrimmerProps) {
 ...
 <label className="mb-1.5 block text-xs font-bold text-muted-foreground">Bắt đầu (mm:ss)</label>
 ...
 />
 <button type="button" onClick={() => handleAdjustStart(-1)} className="rounded-md bg-slate-200 px-2 py-1.5 text-xs font-medium text-foreground transition-colors hover:bg-muted dark:text-muted-foreground">-1s</button>
 <button type="button" onClick={() => handleAdjustStart(1)} className="rounded-md bg-slate-200 px-2 py-1.5 text-xs font-medium text-foreground transition-colors hover:bg-muted dark:text-muted-foreground">+1s</button>
 </div>
 </div>
 <div className="flex-1">
 <label className="mb-1.5 block text-xs font-bold text-muted-foreground">Kết thúc (mm:ss)</label>
 <div className="flex items-center gap-2">
 <input
 type="text"
 value={endText}
 ...
 className="w-20 rounded-lg border border-border bg-card px-3 py-1.5 text-center font-mono text-sm text-foreground outline-none focus:ring-1 focus:ring-blue-500 dark:text-muted-foreground"
 />
 <button type="button" onClick={() => handleAdjustEnd(-1)} className="rounded-md bg-slate-200 px-2 py-1.5 text-xs font-medium text-foreground transition-colors hover:bg-muted dark:text-muted-foreground">-1s</button>
 <button type="button" onClick={() => handleAdjustEnd(1)} className="rounded-md bg-slate-200 px-2 py-1.5 text-xs font-medium text-foreground transition-colors hover:bg-muted dark:text-muted-foreground">+1s</button>
 </div>
 </div>
 </div>
 ...
 <button type="button" onClick={onCancel} className="rounded-xl bg-muted px-4 py-2 text-sm font-bold text-muted-foreground transition-colors hover:bg-muted dark:text-muted-foreground">
 Hủy
 </button>
 </div>
 </div>
...
function Step1({ level, setLevel, title, setTitle, description, setDescription, timeLimit, setTimeLimit, mondai, setMondai, onNext, loading }: Step1Props) {
 ...
 {/* Level */}
 <div>
 <label className="block text-sm font-semibold text-foreground dark:text-muted-foreground mb-2">Cấp độ JLPT</label>
 ...
 <div>
 <label className="block text-sm font-semibold text-foreground dark:text-muted-foreground mb-2">Tiêu đề bài nghe</label>
 ...
 <div>
 <label className="block text-sm font-semibold text-foreground dark:text-muted-foreground mb-2">Thời gian làm bài (Phút)</label>
 ...
 <p className="text-xs text-muted-foreground mt-1">Tổng thời gian cho toàn bộ các phần thi bên dưới.</p>
 </div>
 </div>

 <div className="space-y-4">
 <div>
 <label className="block text-sm font-semibold text-foreground dark:text-muted-foreground mb-2">Mô tả đề thi</label>
 <textarea
 value={description}
 ...
 <label className="text-sm font-semibold text-foreground dark:text-muted-foreground">Cấu hình Mondai (Dạng bài tập)</label>
 <span className="text-xs text-muted-foreground">Tổng số câu hỏi: <span className="font-bold text-foreground dark:text-muted-foreground">{totalQ}</span></span>
 </div>
...
function Step2({ questions, examId, onQuestionsChange, onBack }: Step2Props) {
 ...
 <div>
 <p className="text-base font-semibold text-foreground dark:text-muted-foreground">Không còn câu hỏi nào</p>
 <p className="mt-2 text-sm text-muted-foreground">Quay lại bước cấu hình để tạo lại cấu trúc đề thi.</p>
 </div>
 </div>
 )
 }
 ...
 <p className="text-xs font-bold text-muted-foreground uppercase tracking-[0.18em]">Danh sách câu hỏi</p>
 <span className="rounded-full bg-muted px-3 py-1 text-xs font-bold text-muted-foreground">{questions.length} câu</span>
 </div>
 ...
 <div className="flex items-center gap-2">
 <span className="rounded-full bg-muted px-3 py-1 text-xs font-bold text-muted-foreground">{groupQs.length} câu</span>
 <button
 type="button"
 ...
 <div>
 <p className="text-xs text-muted-foreground uppercase tracking-wide font-medium">Đang chỉnh sửa</p>
 <div className="mt-1 flex items-center gap-3">
 <div className="inline-flex items-center gap-2 rounded-xl border border-border bg-muted px-3 py-1.5">
 <span className="text-sm font-bold text-foreground dark:text-muted-foreground">{q.mondai_group}</span>
 <span className="text-muted-foreground">-</span>
 <span className="text-sm font-bold text-foreground dark:text-muted-foreground">Câu</span>
 ...
 <span className="w-5 text-center text-sm font-bold text-card-foreground">{q.question_number}</span>
 <button
 type="button"
 ...
 <span className="mr-1 text-xs font-bold text-foreground dark:text-muted-foreground">IRT</span>
 {[1, 2, 3, 4, 5].map((star) => (
 <button
 key={star}
 type="button"
 ...
 <p className="text-sm font-semibold text-foreground dark:text-muted-foreground">File âm thanh (Audio)</p>
 </div>
 {isEditingAudio ? (
 <AudioTrimmer
 ...
 Nội dung câu hỏi <span className="text-muted-foreground font-normal">– Câu hỏi được đọc cuối bài</span>
 </label>
 ...
 <div>
 <label className="block text-xs font-semibold text-muted-foreground mb-1.5">
 Script (Tiếng Nhật) <span className="text-muted-foreground font-normal">– Hiển thị khi xem lại</span>
 </label>
 ...
 {!error && <p className="text-xs text-muted-foreground">Lần sửa cuối: {new Date().toLocaleTimeString('vi', { hour: '2-digit', minute: '2-digit' })} hôm nay</p>}
 <div className="flex items-center gap-3 ml-auto">
 <button onClick={onBack} className="px-4 py-2 text-sm text-muted-foreground hover:bg-accent hover:text-accent-foreground rounded-lg transition-colors">
 Quay lại
 </button>
...
function Step3({ questions, examTitle, onBack, onPublish, publishing }: Step3Props) {
 ...
 <p className="mt-1 text-sm text-muted-foreground">Đã hoàn thành: <span className="font-semibold text-blue-600 dark:text-blue-400">{savedCount}/{questions.length}</span> câu hỏi</p>
 </div>
 <div className="rounded-xl border border-border bg-card p-10 text-center">
 <p className="text-base font-semibold text-foreground dark:text-muted-foreground">Không có câu hỏi để xem lại</p>
 </div>
 </div>
 )
 }
 ...
 <div className="rounded-xl border border-blue-100 bg-gradient-to-r from-blue-50 to-indigo-50 p-5 dark:border-blue-800/50 dark:from-blue-900/20 dark:to-indigo-900/20">
 <h3 className="text-base font-bold text-card-foreground">{examTitle}</h3>
 <p className="text-sm text-muted-foreground mt-1">Đã hoàn thành: <span className="font-semibold text-blue-600 dark:text-blue-400">{savedCount}/{questions.length}</span> câu hỏi</p>
 </div>
 ...
 <div className="flex items-center justify-between">
 <p className="text-xs font-bold uppercase tracking-[0.18em] text-muted-foreground">Danh sách câu hỏi</p>
 <span className="rounded-full bg-muted px-3 py-1 text-xs font-bold text-muted-foreground">{questions.length} câu</span>
 </div>
 ...
 <div>
 <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Xem lại</p>
 <div className="mt-1 flex items-center gap-3">
 <div className="inline-flex items-center gap-2 rounded-xl border border-border bg-muted px-3 py-1.5">
 <span className="text-sm font-bold text-foreground dark:text-muted-foreground">{q.mondai_group}</span>
 <span className="text-muted-foreground">-</span>
 <span className="text-sm font-bold text-foreground dark:text-muted-foreground">Câu {q.question_number}</span>
 </div>
 </div>
 </div>
 <div className="flex items-center gap-1 rounded-xl border border-border bg-muted px-2.5 py-1.5">
 <span className="mr-1 text-xs font-bold text-foreground dark:text-muted-foreground">IRT</span>
 {[1, 2, 3, 4, 5].map((star) => (
 <span key={star} className={`${(q.difficulty || 3) >= star ? 'text-amber-400' : 'text-muted-foreground dark:text-muted-foreground'}`}>
 ...
 <div>
 <label className="mb-1.5 block text-xs font-semibold text-muted-foreground">Nội dung câu hỏi</label>
 <textarea value={q.question_text} readOnly rows={3} className="w-full resize-none rounded-lg border border-border bg-muted/50 px-3 py-2.5 text-sm text-foreground outline-none dark:text-muted-foreground" />
 </div>
 ...
 {q.answers.map((ans, ai) => (
 <div key={ans.id} className="flex items-center gap-2">
 <div className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full border-2 ${ans.is_correct ? 'border-blue-500 bg-blue-500/15' : 'border-border'}`}>
 {ans.is_correct ? <span className="h-2 w-2 rounded-full bg-blue-500" /> : null}
 </div>
 <div className={`flex min-h-[38px] flex-1 items-center rounded-xl border px-3 ${ans.is_correct ? 'border-blue-500 bg-blue-50 dark:border-blue-600 dark:bg-blue-900/20' : 'border-border bg-muted '}`}>
 <span className="mr-2.5 text-sm font-bold text-muted-foreground">{String.fromCharCode(65 + ai)}.</span>
 <input value={ans.content} readOnly className="w-full bg-transparent text-[13px] font-medium text-foreground outline-none dark:text-muted-foreground" />
 </div>
 </div>
 ))}
 </div>
 ...
 <button onClick={onBack} className="flex items-center gap-2 px-4 py-2.5 text-sm text-muted-foreground hover:bg-accent hover:text-accent-foreground rounded-lg transition-colors">

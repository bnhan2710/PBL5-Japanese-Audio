...
export default function ExamPrintPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    Promise.all([
      examClient.getExam(examId),
      examClient.getExamQuestions(examId),
    ])
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-stone-100 flex items-center justify-center">
        <div className="flex items-center gap-3 rounded-2xl border border-stone-200 bg-white px-6 py-4 text-stone-700 shadow-sm">
    ...
    return (
      <div className="min-h-screen bg-stone-100 flex items-center justify-center px-6">
        <div className="max-w-lg rounded-3xl border border-rose-200 bg-white p-8 shadow-sm">
          <p className="text-sm font-semibold uppercase tracking-[0.25em] text-rose-500">PDF Exam</p>
          <h1 className="mt-3 text-2xl font-bold text-stone-900">Không thể mở đề thi</h1>
          <p className="mt-3 text-sm leading-6 text-stone-600">{error || 'Dữ liệu đề thi không hợp lệ'}</p>
          <button
  ...
  return (
    <div
      className="min-h-screen bg-[linear-gradient(180deg,#f8f4ee_0%,#efe6dc_100%)] py-8 text-stone-900 print:bg-white print:py-0"
    ...
    >
      ...
      <div className="print-hidden mx-auto mb-6 flex max-w-5xl items-center justify-between px-6">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.3em] text-rose-500">Exam PDF Studio</p>
          <h1 className="mt-2 text-2xl font-bold text-stone-900">Xuất đề thi sang PDF</h1>
          <p className="mt-1 text-sm text-stone-600">Trang này được tối ưu để dùng tính năng Save as PDF của trình duyệt.</p>
          <p className="mt-2 text-xs text-stone-500">
            Nếu còn hiện URL ở mép trang khi in, hãy bỏ chọn mục <span className="font-semibold">Headers and footers</span> trong hộp thoại in của trình duyệt.
          </p>
        </div>
        <div className="flex items-center gap-3">
          <Link
            to="/exam"
            className="inline-flex items-center gap-2 rounded-xl border border-stone-300 bg-white px-4 py-2.5 text-sm font-semibold text-stone-700 transition hover:border-stone-400 hover:bg-stone-50"
          >
            <ArrowLeft className="h-4 w-4" />
            Quay lại
          </Link>
      ...
      <main className="print-sheet mx-auto max-w-5xl overflow-hidden rounded-[32px] border border-stone-200 bg-white shadow-[0_30px_80px_rgba(120,83,50,0.15)]">
        <section className="relative overflow-hidden border-b border-stone-200 bg-[radial-gradient(circle_at_top_left,#f97316_0,#fb7185_48%,#fff7ed_100%)] px-8 py-10 text-white">
          ...
          <div className="relative">
            <p className="text-xs font-semibold uppercase tracking-[0.35em] text-white/80">Japanese Listening Exam</p>
            ...
            <div className="mt-8 grid gap-3 sm:grid-cols-3">
              <div className="rounded-2xl border border-white/30 bg-white/15 px-4 py-3 backdrop-blur-sm">
                <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-white/70">Thời lượng</p>
              ...
              <div className="rounded-2xl border border-white/30 bg-white/15 px-4 py-3 backdrop-blur-sm">
                <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-white/70">Số câu hỏi</p>
                <p className="mt-2 text-base font-semibold">{questions.length} câu</p>
              </div>
              <div className="rounded-2xl border border-white/30 bg-white/15 px-4 py-3 backdrop-blur-sm">
                <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-white/70">Ngày tạo</p>
                <p className="mt-2 flex items-center gap-2 text-base font-semibold">
        ...
        <section className="border-b border-stone-200 bg-stone-50 px-8 py-6">
          <h2 className="text-sm font-bold uppercase tracking-[0.25em] text-stone-500">Hướng dẫn</h2>
          <div className="mt-4 grid gap-3 md:grid-cols-3">
            <div className="rounded-2xl border border-stone-200 bg-white p-4">
              <p className="text-sm font-semibold text-stone-900">1. Nghe kỹ đoạn audio</p>
              <p className="mt-2 text-sm leading-6 text-stone-600">Mỗi câu hỏi được thiết kế cho kỹ năng nghe hiểu tiếng Nhật theo từng phần mondai.</p>
            </div>
            <div className="rounded-2xl border border-stone-200 bg-white p-4">
              <p className="text-sm font-semibold text-stone-900">2. Chọn một đáp án đúng nhất</p>
              <p className="mt-2 text-sm leading-6 text-stone-600">Đề in ra giữ khoảng trắng hợp lý để người làm bài dễ đọc và ghi chú trực tiếp.</p>
            </div>
            <div className="rounded-2xl border border-stone-200 bg-white p-4">
              <p className="text-sm font-semibold text-stone-900">3. Đối chiếu đáp án ở cuối file</p>
              <p className="mt-2 text-sm leading-6 text-stone-600">Phần đáp án được tách riêng giúp dùng chung cho cả bản thi và bản chấm.</p>
            </div>
          </div>
        </section>
            ...
            {Object.entries(groupedQuestions).map(([groupName, groupQuestions], groupIndex) => (
              <section key={groupName} className="group-block">
                <div className="mb-5 flex items-end justify-between gap-4 border-b border-stone-200 pb-3">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.25em] text-rose-500">Phần {groupIndex + 1}</p>
                  ...
                  {groupQuestions.map((question) => {
                    ...
                    return (
                      <article
                        ...
                        className="question-block rounded-[28px] border border-stone-200 bg-[linear-gradient(180deg,#fffefb_0%,#fff8f1_100%)] p-6"
                      >
                        <div className="flex items-start justify-between gap-4">
                          <div>
                            <p className="text-xs font-semibold uppercase tracking-[0.25em] text-stone-500">Question</p>
                        ...
                        {question.explanation && (
                          <div className="mt-5 rounded-2xl border border-stone-200 bg-white px-4 py-4">
                            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-stone-500">Script</p>
                        ...
                        {question.image_url && (
                          <div className="mt-5">
                            <img
                              ...
                              className="max-h-80 rounded-2xl border border-stone-200 object-contain"
                            />
                          </div>
                        )}
                        ...
                        <div className="mt-6 grid gap-3">
                          {question.answers.map((answer, answerIndex) => (
                            <div
                              ...
                              className="answer-card flex items-start gap-4 rounded-2xl border border-stone-200 bg-white px-4 py-3"
                            >
                              ...
                              <div className="min-w-0 flex-1">
                                ...
                                {answer.image_url && (
                                  <img
                                    ...
                                    className="mt-3 max-h-52 rounded-2xl border border-stone-200 object-contain"
                                  />
                                )}
                              </div>
                            </div>
                        ...
                        <div className="mt-6 flex items-center justify-between gap-3 rounded-2xl bg-stone-900 px-4 py-3 text-xs text-stone-200">
                          ...
                          <span>Đáp án đúng: {correctAnswerIndex >= 0 ? String.fromCharCode(65 + correctAnswerIndex) : 'Chưa thiết lập'}</span>
                        </div>
                      </article>
                    )
                  })}
        ...
        <section className="page-break border-t border-dashed border-stone-300 bg-stone-50 px-8 py-8">
          <div className="mb-6 flex items-center justify-between gap-4">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.3em] text-rose-500">Answer Key</p>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {Object.entries(groupedQuestions).map(([groupName, groupQuestions]) => (
              <section key={groupName} className="rounded-[28px] border border-stone-200 bg-white p-5">
                ...
                <div className="mt-4 space-y-2">
                  {groupQuestions.map((question) => {
                    ...
                    return (
                      ...
                      >
                        <span className="font-medium text-stone-700">Câu {question.question_number || '?'}</span>
                        <span className="rounded-full bg-stone-900 px-3 py-1 text-xs font-bold text-white">
                          {correctAnswerIndex >= 0 ? String.fromCharCode(65 + correctAnswerIndex) : 'N/A'}
                        </span>
                      </div>
                    )
                  })}

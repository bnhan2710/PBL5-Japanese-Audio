      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8">
        {loading && !data ? (
          ...
          <div className="flex flex-col items-center justify-center h-64 text-slate-500">
            ...
            <p className="text-lg font-medium text-slate-600 dark:text-slate-400">Chưa có dữ liệu bài thi</p>
          ...
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {data.results.map((result) => (
                <div
                  ...
                  className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-sm hover:shadow-md transition-shadow group flex flex-col"
                >
                  <div className="p-5 flex-1 space-y-4">
                    {/* Header */}
                    <div>
                      <h3 className="text-base font-bold text-slate-800 dark:text-slate-100 line-clamp-2">
                        {result.exam_title || 'Đề thi không xác định'}
                      </h3>
                    </div>

                    {/* Stats Grid */}
                    <div className="grid grid-cols-2 gap-3 py-3 border-y border-slate-100 dark:border-slate-700/50">
                      <div className="flex flex-col">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <span className={`text-lg font-bold ${
                            result.score !== null && result.score <= 19 ? 'text-rose-600 dark:text-rose-500' : 'text-indigo-600 dark:text-indigo-400'
                          }`}>
            ...
            {/* Pagination Controls */}
            {data.total_pages > 1 && (
              <div className="mt-8 flex items-center justify-between bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-4 shadow-sm">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    disabled={page <= 1}
                    className="px-4 py-2 text-sm font-medium border border-slate-200 dark:border-slate-700 rounded-lg text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                  >
                    Trang trước
                  </button>
                  <button

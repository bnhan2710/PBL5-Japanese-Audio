...
export function CreateUserModal({ isOpen, onClose, onSubmit }: CreateUserModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Reset
      setFormData({ email: '', username: '', role: 'user', password: '', first_name: '', last_name: '' });
      setAvatarFile(null);
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-card rounded-xl shadow-xl max-w-md w-full p-6 relative border border-border">
        ...
        <h2 className="text-lg font-bold text-foreground mb-5">Tạo người dùng mới</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            ...
            >
              {uploading ? 'Đang tạo...' : 'Tạo người dùng'}
            </button>
          </div>
        </form>
      </div>

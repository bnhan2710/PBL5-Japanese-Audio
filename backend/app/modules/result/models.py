...
class UserResult(Base):
    ...
    exam_id = Column(
        UUID(as_uuid=True), ForeignKey("exams.exam_id", ondelete="SET NULL"), nullable=True
    )
    contest_id = Column(
        UUID(as_uuid=True), ForeignKey("contests.contest_id", ondelete="SET NULL"), nullable=True
    )
    ...
    competency_analysis = relationship(
        "CompetencyAnalysis", back_populates="result", uselist=False, cascade="all, delete-orphan"
    )
...
class CompetencyAnalysis(Base):
    ...
    result_id = Column(
        UUID(as_uuid=True),
        ForeignKey("user_results.result_id", ondelete="CASCADE"),
        nullable=False,
        unique=True,
    )
